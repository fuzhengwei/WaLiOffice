...
pub fn list_by_owner(pool: &DbPool, owner_id: &str) -> AppResult<Vec<TaskRow>> {
    ...
    let rows = stmt.query_map(params![owner_id], |row| Ok(TaskRow {
...
pub fn create(pool: &DbPool, owner_id: &str, title: &str, description: Option<&str>, priority: &str, due_date: Option<&str>, project_id: Option<&str>, tags: Option<&str>) -> AppResult<TaskRow> {
    let conn = pool.get().map_err(|e| anyhow::anyhow!(e))?;
    let id = uuid::Uuid::new_v4().to_string();
    let now = chrono::Utc::now().to_rfc3339();
    let max_order: i64 = conn.query_row(
        "SELECT COALESCE(MAX(order_col), 0) FROM tasks WHERE owner_id = ?1",
        params![owner_id], |r| r.get(0),
    ).unwrap_or(0);
    conn.execute(
    ...
    Ok(TaskRow {
        id, owner_id: owner_id.to_string(), title: title.to_string(), description: description.map(String::from),
        status: "todo".into(), priority: priority.to_string(), due_date: due_date.map(String::from),
        project_id: project_id.map(String::from), tags: tags.map(String::from),
        order_col: max_order + 1, created_at: now.clone(), updated_at: now,
    })
}

pub fn update(pool: &DbPool, id: &str, owner_id: &str, title: Option<&str>, description: Option<&str>, status: Option<&str>, priority: Option<&str>, due_date: Option<&str>, tags: Option<&str>, order: Option<i64>) -> AppResult<Option<TaskRow>> {
    let conn = pool.get().map_err(|e| anyhow::anyhow!(e))?;
    let now = chrono::Utc::now().to_rfc3339();
    let mut updates: Vec<String> = vec!["updated_at = ?1".to_string()];
    let mut params_vec: Vec<Box<dyn rusqlite::ToSql>> = vec![Box::new(now.clone())];
    let mut idx = 2;
    if let Some(v) = title { updates.push(format!("title = ?{idx}")); params_vec.push(Box::new(v.to_string())); idx += 1; }
    if let Some(v) = description { updates.push(format!("description = ?{idx}")); params_vec.push(Box::new(v.to_string())); idx += 1; }
    if let Some(v) = status { updates.push(format!("status = ?{idx}")); params_vec.push(Box::new(v.to_string())); idx += 1; }
    if let Some(v) = priority { updates.push(format!("priority = ?{idx}")); params_vec.push(Box::new(v.to_string())); idx += 1; }
    if let Some(v) = due_date { updates.push(format!("due_date = ?{idx}")); params_vec.push(Box::new(v.to_string())); idx += 1; }
    if let Some(v) = tags { updates.push(format!("tags = ?{idx}")); params_vec.push(Box::new(v.to_string())); idx += 1; }
    if let Some(v) = order { updates.push(format!("order_col = ?{idx}")); params_vec.push(Box::new(v.to_string())); idx += 1; }
    updates.push(format!("owner_id = ?{idx}")); params_vec.push(Box::new(owner_id.to_string())); idx += 1;
    let sql = format!("UPDATE tasks SET {} WHERE id = ?{}", updates.join(", "), idx);
...
pub fn delete(pool: &DbPool, id: &str, owner_id: &str) -> AppResult<bool> {
    let conn = pool.get().map_err(|e| anyhow::anyhow!(e))?;
    let affected = conn.execute("DELETE FROM tasks WHERE id = ?1 AND owner_id = ?2", params![id, owner_id])?;
    Ok(affected > 0)
}
